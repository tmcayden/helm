...
import { TMUX_SNIPPET, ZSH_SNIPPET } from './snippets.ts'
...
describe.skipIf(!present('tmux', ['-V']))('helm.tmux on a throwaway server', () => {
  it('binds the six rows and appends the status segment once however often it is sourced', () => {
    ...
    try {
      ...
      for (const key of ['p', 'h', 'e', 'c', 'u', 'Escape']) expect(keys).toMatch(new RegExp(`-T helm\\s+${key}\\s`))
